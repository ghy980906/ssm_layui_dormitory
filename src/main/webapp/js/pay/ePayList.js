...
layui.config({
    ...
    table.render({
        ...
        cols:[[ //表头
            ...
            {field:'payType',title:'缴费方式',align:'center',width:110,templet:'#payType'},
    ...
    table.on('tool(payList)',function (obj) {
        ...
        if(obj.event=='del')
        {
            ...
            layer.confirm('真的删除吗？',function (index) {
                $.ajax({
                    url:ctx+"/pay/delWPay/"+data.payId,
                    type:"get",
                    success:function (d) {
                        if(d.code==0)
                        {
                            table.reload('payList',{});//重新发送请求 表格重载
                        }
                        else
                        {
